...
const actions = {
  async fetchInfo({ commit }) {
    try {
      const response = await axios.get("v1/admin/projects");
  ...
  async createInfo({ commit }, newInfo) {
    try {
      const response = await axios.post("v1/admin/projects", newInfo);
  ...
  async getInfoById({ commit }, id) {
    try {
      const response = await axios.get(`v1/admin/projects/${id}`);
  ...
  async deleteInfo({ commit }, id) {
    try {
      await axios.delete(`/v1/admin/projects/${id}`, {}); // Xóain
